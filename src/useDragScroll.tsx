...
const useDragScroll = (containerRef: RefObject<HTMLElement>) => {
  ...
  const containerScrollWidth = useRef<number>(0);
  const containerClientWidth = useRef<number>(0);
  ...
  const getClientX = useCallback((e: any) => {
  ...
  const onScrollEnd = useCallback(
    (e: any) => {
      ...
      if (containerX.current > 0) {
        ...
        setTranslateX(
          containerClientWidth.current - containerScrollWidth.current - 16
        ); // 맨 오른쪽 패딩 16px 살리기
        containerRef.current!.style.transition = `all 0.3s ease`;
        containerX.current =
          containerClientWidth.current - containerScrollWidth.current;
